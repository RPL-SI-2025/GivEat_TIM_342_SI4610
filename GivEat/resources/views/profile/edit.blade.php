<x-app-layout>
		<div class="flex min-h-screen">
				...
				<main class="flex-1 p-10">
						<h1 class="mb-1 text-2xl font-semibold">Profile Saya</h1>
						<p class="mb-8 text-gray-500">Atur detail profile kamu.</p>

						<!-- Profile Section -->
						<div id="profileContent" class="max-w-6xl rounded-[50px] bg-white p-8">


								<div class="mb-8 flex items-center space-x-6">
										<img src="{{ Auth::user()->image ? Storage::url(Auth::user()->image) : asset('profile/default.png') }}"
												alt="Profile" class="h-24 w-24 rounded-full object-cover">

										<button type="button" onclick="document.getElementById('uploadModal').classList.remove('hidden')"
												class="rounded-full bg-green-800 px-5 py-2 text-white hover:bg-green-900">Ganti foto profil</button>
										<button type="button" onclick="document.getElementById('deletePhotoModal').classList.remove('hidden')"
												class="rounded-full bg-red-300 px-5 py-2 font-semibold text-red-700 hover:bg-red-500 hover:text-white">Hapus
												Foto</button>
								</div>
								...
								<form method="POST" action="{{ route('profile.update') }}" class="space-y-6">
										...
										<div>
												...
												<input type="text" name="name" value="{{ old('name', Auth::user()->name) }}"
														class="w-full rounded-full border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-800"
														required />
												@error('name')
										...
										<div>
												...
												<input type="text" name="gender" value="{{ old('gender', Auth::user()->gender) }}"
														class="w-full rounded-full border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-800" />
												@error('gender')
														<span class="text-sm text-red-600">{{ $message }}</span>
												@enderror
										...
										<div>
												<label class="mb-1 block text-sm font-medium">Domisili</label>
						...
						{{-- di pindah ke halaman lain agar lebih modular --}}
				</main>
		</div>



		<!-- Tambahkan modal konfirmasi baru sebelum </body>, setelah modal upload -->
		<div id="deletePhotoModal" class="fixed inset-0 flex hidden items-center justify-center bg-black bg-opacity-50">
				<div class="w-full max-w-md rounded-[50px] bg-white p-8">
						<h2 class="mb-4 text-xl font-semibold">Hapus Foto Profil</h2>
						<p class="mb-6 text-gray-600">Apakah Anda yakin ingin menghapus foto profil? Foto akan kembali ke default.</p>
						<form method="POST" action="">
								@csrf
								@method('DELETE')
								<div class="flex justify-end space-x-4">
										<button type="button" onclick="document.getElementById('deletePhotoModal').classList.add('hidden')"
												class="rounded-full bg-gray-300 px-5 py-2 text-gray-700 hover:bg-gray-400">Batal</button>
										<button type="submit" class="rounded-full bg-red-600 px-5 py-2 text-white hover:bg-red-700">Hapus</button>
								</div>
						</form>
				</div>
		</div>
		...
		<!-- Modal for Photo Upload (Tambahkan sebelum </body>) -->
		<div id="uploadModal" class="fixed inset-0 flex hidden items-center justify-center bg-black bg-opacity-50">
				<div class="w-full max-w-md rounded-[50px] bg-white p-8">
						<h2 class="mb-6 text-xl font-semibold">Unggah Foto Profil</h2>
						<form method="POST" action="" enctype="multipart/form-data">
								@csrf
								<div class="mb-4">
										<label class="mb-1 block text-sm font-medium">Pilih Foto</label>
								...
								<div class="flex justify-end space-x-4">
										<button type="button" onclick="document.getElementById('uploadModal').classList.add('hidden')"
												class="rounded-full bg-gray-300 px-5 py-2 text-gray-700 hover:bg-gray-400">Batal</button>
										<button type="submit"
												class="rounded-full bg-green-800 px-5 py-2 text-white hover:bg-green-900">Unggah</button>
								</div>
						</form>
				</div>
		</div>
